...
new_test(creating_rays) {
        nucray::camera cam(nucray::vector(0,0,0), nucray::vector(1,0,0), nucray::vector(0,1,0),1.0f, 4,4);
        auto rays_device = cam.get_rays();
        thrust::host_vector<nucray::ray> rays_host(rays_device);
        for (auto &ray: rays_host) {
                std::cout<<"ray origin:" << ray.start_position.x << " " << ray.start_position.y << " " << ray.start_position.z
                         <<"ray direction:" << ray.direction.x << " " << ray.direction.y << " " << " " << ray.direction.z << std::endl;
        }
}
